...
var Map = /** @class */ (function () {
	...
	Map.prototype.init = function () {
		...
		var zoom = d3__namespace
			...
			.wheelDelta(function (event) {
				if (event.deltaY < 0) {
					if (_this.zoomLevel === 0) return Math.log2(1);
					_this.zoomLevel--;
					return Math.log2(
						config.zooms[_this.zoomLevel] / config.zooms[_this.zoomLevel + 1]
					);
				} else {
					if (_this.zoomLevel === 19) return Math.log2(1);
					_this.zoomLevel++;
					return Math.log2(
						config.zooms[_this.zoomLevel] / config.zooms[_this.zoomLevel - 1]
					);
		...
		this.zoom = zoom;
		zoom.scaleBy(svg, config.zooms[this.zoomLevel]);
			...
			.on("click", function (e) {
				...
				if (_this.options.onClick) {
					_this.options.onClick(
						e,
						projection.invert(d3__namespace.pointer(e, g.node()))
					);
				}
			})
	...
	Map.prototype.showLayer = function (layer) {
		if (layer === undefined) {
			this.layers.forEach(function (i) {
				return i.show();
			});
		} else {
			layer.show();
	...
	Map.prototype.focusOnView = function (coords) {
		...
		var min = [];
		var max = [];
		(_a = d3__namespace.extent(projectionCoords, function (d) {
			return d[0];
		})),
			(min[0] = _a[0]),
			(max[0] = _a[1]);
		(_b = d3__namespace.extent(projectionCoords, function (d) {
			return d[1];
		})),
			(min[1] = _b[0]),
			(max[1] = _b[1]);
	...
	Map.prototype.setLevel = function (level) {
		...
		this.zoom.scaleBy(d3__namespace.select(this.svg), this.zoomLevel);
	};
	return Map;
})();
