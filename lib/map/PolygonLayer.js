...
var PolygonLayer = /** @class */ (function (_super) {
    ...
    PolygonLayer.prototype.draw = function () {
        var _this = this;
        var _a = this.formatData(this.data), pathData = _a[0], nameData = _a[1];
        ...
        pathGroup
            ...
            _this.clickTimer = setTimeout(function () {
                if (_this.clickCount % 2 === 1) {
                    ...
                    if (selectable) {
                        _this.combineIndex(_this.selectIndex, d.properties.index, index);
            ...
            if (hasHover) {
                ...
                var index_1 = coordinates.findIndex(function (i) {
                    return d3__namespace.polygonContains(i, targetCoord_1);
                });
                if (index_1 > -1) {
                    var hoverData = _this.formatData(_this.data, function (e, outerIndex, innerIndex) {
                        if (outerIndex === d.properties.index && innerIndex === index_1) {
                            return true;
                        }
                        return false;
                    });
                    _this.drawHoverLayer(hoverData);
                }
            }
        })
            .on("mouseleave", function () {
            _this.hoverLayer.select("*").remove();
        })
        ...
        nameGroup
            ...
            .attr("dx", function (d) {
            var width = _this.calcuteTextWidth(d.name);
            return -width / 2;
        })
            .text(function (d) { return d.name; });
    };
    PolygonLayer.prototype.combineIndex = function (idxs, index, idx) {
        var _a;
        if (idxs.has(index)) {
    ...
    PolygonLayer.prototype.drawHoverLayer = function (data) {
        var _this = this;
        this.hoverLayer.selectAll("*").remove();
        var pathGroup = this.hoverLayer.append("g");
        var nameGroup = this.hoverLayer.append("g");
        var pathData = data[0], nameData = data[1];
        pathGroup
            .selectAll("path")
            .data(pathData)
            .enter()
    ...
    PolygonLayer.prototype.formatData = function (data, dataFilter) {
        ...
        var pathData = data.reduce(function (pre, cur, idx) {
            ...
            data.forEach(function (j, innerIndex) {
                ...
                if (j.name) {
                    var nameStyle = _tslib.__assign(_tslib.__assign({}, j.nameStyle), defaultNameStyle);
                    nameData.push({
                        name: j.name,
                        fontSize: nameStyle.fontSize,
                        fontWeight: nameStyle.fontWeight,
                        color: nameStyle.color,
                        coordinate: _this.projection(d3__namespace.polygonCentroid(j.coordinates)),
                    });
                }
                if (j.reverseCoords) {
